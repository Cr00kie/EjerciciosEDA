...
public:
   ...
   // Ej5-2
   void frontera(std::vector<T>& out){
      frontera(raiz, out);
   }
...
private:
   ...
   int nodos(Link raiz){
      if(!raiz) return 0;
      else return nodos(raiz->left) + nodos(raiz->right) + 1;
   }

   // Ej5-1
   int hojas(Link raiz){
      if(!raiz) return 0;
      if(!raiz->right && !raiz->left) return 1;
      return hojas(raiz->right) + hojas(raiz->left);
   }
   ...
   int altura(Link raiz){
      ...
      return (hDer > hIzq ? hDer : hIzq) + 1;
   }

   void frontera(Link raiz, std::vector<T>& out){
      if(!raiz) return;
      if(!raiz->left && !raiz->right) out.push_back(raiz->elem);
      if(raiz->left) frontera(raiz->left, out);
      if(raiz->right) frontera(raiz->right, out);
   }
